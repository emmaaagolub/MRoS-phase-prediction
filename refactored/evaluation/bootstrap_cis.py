...
Inputs
------
  benchmarking:  evaluation/{REGION}/benchmarking/benchmark_predictions_test.parquet
  ablations:     evaluation/{REGION}/ablation/{config}/shap_values_all.parquet
...
# Outputs root comes from the shared config; MROS_OUTPUTS or --data-dir can
# override it so the same script runs on any machine.
DATA_DIR = Path(os.environ.get("MROS_OUTPUTS", OUTPUT_DIR))
...
def load_benchmark_df(region: str) -> tuple[pd.DataFrame, list[str]]:
    pq = DATA_DIR / f"evaluation/{region}/benchmarking/benchmark_predictions_test.parquet"
...
def load_ablation_test_preds(region: str) -> tuple[dict[str, pd.DataFrame], dict[str, dict]]:
    ...
    """
    root = DATA_DIR / f"evaluation/{region}/ablation"
    if not root.exists():
    ...
    for name, d in cfgs.items():
        ...
        if merged["p_snow_cal"].isna().mean() > 0.02:
            print(f"  WARNING: {name}: {merged['p_snow_cal'].isna().mean():.1%} rows "
                  "unmatched to baseline — skipping")
            continue
...
def main():
    ...
    ap.add_argument("--regions", nargs="+", choices=list(REGION_IDS),
    ...
    for region in args.regions:
        out_dir = DATA_DIR / f"evaluation/{region}/benchmarking/bootstrap"
