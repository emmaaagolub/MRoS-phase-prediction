"""Step 16 — Regenerate every manuscript figure from artifacts already on disk.

Nothing here retrains a model, reruns kriging, or touches raw data. Each figure
reads the saved outputs of an earlier stage, so this can be re-run freely while
iterating on figure design.

Sources read (all read-only):
  outputs/model/<REGION>/            trained model, predictions, SHAP values
  outputs/evaluation/<REGION>/ablation/      per-experiment metrics
  outputs/evaluation/<REGION>/benchmarking/  benchmark comparison
  outputs/evaluation/<REGION>/benchmarking/bootstrap/  confidence intervals
  outputs/compiled/<REGION>/hourly_data/     station and MRoS observations

Output: outputs/figures/<REGION>/<REGION>_<description>[_appendix].png

Naming rules used throughout:
  - Files are <REGION>_<description>.png, with _appendix on figures that
    belong in the supplement rather than the main text.
  - The ablation configuration with every predictor is displayed as "Full
    configuration", never "baseline". The folder name and the saved metric keys
    still read baseline_full and *_baseline_minus_config; those are on-disk
    names and are translated only at display time.
  - Titles are short and Title Case. Bin widths, minimum counts, CI method and
    split belong on the axis labels or in the manuscript caption.

Two figures (study extent and station coverage) fetch state boundaries, city
points and a basemap over the network. They skip themselves with a printed
message if that fetch or the optional geospatial packages are unavailable.
...
# Needed only by the study-extent and station-coverage figures; those two skip
...
from config import (  # noqa: E402
    DATA_DIR, OUTPUT_DIR, PIPELINE_ROOT, REGIONS as REGION_CONFIG, REGION_IDS,
    region_paths,
)
...
# Air-temperature bins, matching benchmarking.py exactly so every temperature
# figure here uses the same bins and the same minimum-count rule. Bins holding
# fewer than MIN_BIN_N pure observations are dropped rather than plotted as a
# noisy estimate, which is why some series stop short of the axis limits.
TAIR_BIN_WIDTH = 1.0
TAIR_BIN_MIN = -8.0
TAIR_BIN_MAX = 8.0
MIN_BIN_N = 20
...
# Display names for the ablation configurations. Folder names must never appear
# in a title, label or legend. Short forms match Table \ref{tab:ablation} in the
# manuscript so the figures and the table read the same way.
CONFIG_DISPLAY_NAME = {
...
# Colours for the phase-diagnostics panels, distinct from the model figures.
DIAG_PHASE_ORDER = ["snow", "mix", "rain"]
DIAG_PHASE_COLORS = {"snow": "#4da6ff", "mix": "#ff69b4", "rain": "#44bb77"}


def feature_label(name: str) -> str:
    """Readable name for a model feature, so raw column names never appear."""
...
def dem_path_for(region: str) -> Path:
    return region_paths(region)["dem_1km"]
...
def load_phase_diagnostics_inputs(region: str):
    """Read the hourly station and MRoS tables written by the compilation stage.

    Timezone handling, hourly averaging and study-area clipping all happened
    when those files were written; nothing is redone here.
    """
...
def _us_states_gdf():
    """US state boundaries (Census TIGER 500k). Fetched over the network."""
    return gpd.read_file("https://www2.census.gov/geo/tiger/GENZ2023/shp/cb_2023_us_state_500k.zip")


@functools.lru_cache(maxsize=1)
def _populated_places_gdf():
    """Natural Earth 1:50m populated places, for city labels. Network fetch."""
...
def safe_savefig(fig, path: Path):
    ...
    plt.close(fig)
    print(f"  wrote {path.relative_to(PIPELINE_ROOT)}")
...
def fig_calibration(region: str):
    ...
    the ablation runner, but reads only the saved
...
def fig_benchmark_delta_forest(region: str):
    ...
    bootstrap in benchmarking/bootstrap)."""
...
def _cluster_bootstrap_tair_bins(region: str, n_boot: int = 500, block_km: float = 30.0,
    ...
    bootstrap_cis.py (make_cluster_codes / build_cluster_index /
...
def fig_benchmark_accuracy_by_tair(region: str):
    ...
    benchmark + model) under benchmarking/ — no recomputation needed.
...
def fig_ablation_comparison(region: str):
    ...
    in the ablation runner, reading the already-saved
    ablation_comparison.csv under ablation/ (produced by load_all_metrics,
...
def _twet_profile(df_f, bin_edges, base_hb, extra_hb, sigma, min_n=10):
    """Reimplementation of _twet_profile_binary from the ablation runner:
    per-Twet-bin F1(snow)/F1(rain)/flag-rate/mix-capture, restricted to
    the full-configuration ablation's saved test-set predictions."""
...
def fig_f1_by_tair(region: str):
    ...
    reflects the ablation runner's story3/twet convention)."""
    pq = ablations_dir(region) / "baseline_full" / "shap_values_all.parquet"
    meta_path = ablations_dir(region) / "baseline_full" / "metrics_summary.json"
    if not pq.exists():
...
def fig_mix_capture_by_wetbulb(region: str):
    ...
    the model's test_full_uncertainty_predictions_combined.parquet.
...
def _nf_profile(df_f, bin_edges, temp_col):
    """Ports _nf_profile from the ablation runner
...
NF_BIN_EDGES = np.arange(-4, 5, 1)  # matches the story5 zoom window in the ablation runner
...
def _fig_story5_compare(region: str, temp_col: str, fname: str):
    ...
    the ablation runner. Both configs are drawn in
    ...
    for cfg in ["baseline_full", "no_mros_loocv"]:
        ...
        if not pq.exists():
            print(f"  [story5:{region}:{fname}] missing {pq} for config '{cfg}', "
                  f"omitting that config from the comparison (rerun the ablation "
...
def fig_confusion_matrix(region: str):
    ...
    Scope, stated explicitly so the figure is not over-read:
      ...
      - Read from benchmarking/benchmark_predictions_test.parquet, an
...
def fig_csi_by_tair(region: str):
    ...
    Computed from benchmarking/benchmark_predictions_test.parquet
...
# the figure that used to be built inline in the compilation notebook
...
def fig_phase_diagnostics_combined(region: str):
    ...
    safe_savefig(fig, out_dir(region) / f"{region}_phase_diagnostics_combined.png")
    ...
    for station_id in station_summary["id"]:
        ...
        try:
            ...
            ax.contour(ta_mesh, el_mesh, density, levels=3,
                       colors=[id_to_color[station_id]], alpha=0.55, linewidths=0.8)
        except Exception:
            # A station with too little spread makes the KDE singular; the
            # scatter below still shows its points.
            pass

    ax.scatter(st_hr["temp_air"], st_hr["elev"],
               c=[id_to_color.get(sid, "gray") for sid in st_hr["id"]],
               s=1, alpha=0.08, rasterized=True)
    ax.axvline(0, color="black", lw=0.8, ls="--", alpha=0.5, label="0 °C")
    ax.axvline(2, color="gray", lw=0.8, ls=":", alpha=0.5, label="2 °C")
...
def _draw_station_distribution_panel(ax, st_hr):
    ...
    colors = {"temp_air": "#e05252", "temp_wet": "#5296e0", "rh": "#52b452"}
    ...
    ax.set(xlabel="Temperature (°C)", ylabel="Density",
           title="Observed Distributions")

    ax_rh = ax.twiny()
    ax_rh.hist(st_hr["rh"].dropna(), bins=50, color=colors["rh"], alpha=0.35,
...
def _draw_station_map_panel(ax, region, station_summary, id_to_color):
    ...
    dem_gdf.plot(ax=ax, facecolor="none", edgecolor="red", linewidth=1.5, zorder=3)
    ...
    ax.set_xticks(xticks, [f"{lon:.2f}°" for lon in lons], fontsize=7)
    ax.set_yticks(yticks, [f"{lat:.2f}°" for lat in lats], fontsize=7)
    ax.set(xlabel="Longitude", ylabel="Latitude", title="Station Locations")

    for hours, size in [(500, 30), (5000, 100),
                        (station_summary["n_hours"].max(), 200)]:
        ax.scatter([], [], c="gray", s=size, alpha=0.8, label=f"{int(hours):,} hrs")
    ax.legend(title="Hours on record", fontsize=7, title_fontsize=7, loc="lower left")


def fig_station_data_checks(region: str):
    """Three-panel check on the compiled station record.

    Reads only the hourly station table written by the compilation stage.
    """
    ...
    fig.suptitle(
        f"{REGION_CONFIG[region]['label']} — {len(station_summary)} stations, "
        f"{len(st_hr):,} station-hours",
    ...
    if HAVE_GIS:
        ...
        axes[2].set_axis_off()
        axes[2].set_title("Station map needs geopandas / contextily", fontsize=8)
    ...
    for region in args.regions:
        ...
        for fig_key in args.figures:
            try:
                ...
                # One missing artifact should not stop the rest of the set.
                print(f"  [{fig_key}] skipped: {exc}")

    print("Figures written under", OUTPUT_DIR / "figures")
