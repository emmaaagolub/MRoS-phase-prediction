...
Sources (all read-only):
  outputs/model/<REGION>/                              model, predictions, SHAP
  ...
  outputs/compiled/<REGION>/hourly_data/               station and MRoS tables

Output: outputs/figures/<REGION>/<REGION>_<description>[_appendix].png

Naming conventions:
  - Files are <REGION>_<description>.png, with _appendix on supplement figures.
  - The ablation configuration using all predictors is labelled "Full
    configuration" in figures. The folder name and saved metric keys still read
  ...
  - Titles are short and Title Case. Bin widths, minimum counts, CI method and
    split are given on axis labels rather than in titles.

The study-extent and station-coverage figures fetch state boundaries, city
points and basemap tiles over the network. They are skipped with a printed
message if the fetch fails or the optional geospatial packages are missing.
...
# Colours for the phase-diagnostics panels.
...
# The pipeline config resolves these relative to refactored/ (i.e.
# refactored/outputs/compiled/<region>/ and refactored/outputs/figures/<region>),
# but in this repository the compiled hourly tables live under
# outputs/assimilated/<region>/ and the manuscript figures under
# outputs/manuscript_figures/<region>. The original definitions are kept
# commented out immediately below each override; restore them if the pipeline
# outputs are ever moved back under refactored/.
_REPO_OUTPUTS = Path(__file__).resolve().parents[2] / "outputs"
...
def safe_savefig(fig, path: Path):
    ...
    try:
        ...
        # Output written outside PIPELINE_ROOT (see the path overrides above).
        print(f"  wrote {path}")
...
def fig_calibration(region: str):
    ...
    (beta-calibrated). Same content as the ablation runner's
    story2_calibration.png, but reads only the saved
...
def _cluster_bootstrap_tair_bins(region: str, n_boot: int = 500, block_km: float = 30.0,
    ...
    so this file has no dependency on that script.
...
def fig_mix_capture_by_wetbulb(region: str):
    """Mix capture rate per T_wet bin, with the band placement figure as an
    appendix output.
    ...
    That file carries no clustering key, so the CI is a per-bin binomial
    normal approximation rather than a cluster bootstrap. Sample size is
    ...
    # Main-body figure; the per-bin capture-rate figure is the appendix one.
...
def fig_confusion_matrix(region: str):
    ...
    Cells are annotated with the count and the row-normalised percentage
    (recall per observed class). Accuracy and POD/FAR are printed in the
    panel subtitle.
    """
    pq = benchmarking_dir(region) / "benchmark_predictions_test.parquet"
    if not pq.exists():
...
def _csi_from_counts(hits, misses, false_alarms):
    ...
    CSI is undefined there rather than 0."""
...
def fig_csi_by_tair(region: str):
    ...
    CSI, or threat score, is hits / (hits + misses + false alarms) for the
    event class, so unlike accuracy it is not inflated by class imbalance
    within a bin.

    Conventions match the other T_air figures: TAIR_BIN_EDGES bins ({TAIR_BIN_WIDTH} °C
...
#
# All four panels in one 2x2 figure, labelled A) through D), built from the
# same _draw_* functions as the standalone versions. Includes the dashed
# AOI-to-DEM callout lines between panels A and B, drawn only if both
# panels rendered.
# ---------------------------------------------------------------------------
